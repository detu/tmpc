...
namespace tmpc :: testing
{
	...
	std::istream& operator>>(std::istream& is, blaze::Matrix<MT, SO>& m)
	{
	...
	std::istream& operator>>(std::istream& is, blaze::Vector<VT, TF>& v)
	{
	...
	TEST_F(ExplicitRungeKutta4Test, testOdeCorrect)
	{
		TestPoint p;
	...
	TEST_F(ExplicitRungeKutta4Test, testOdeQCorrect)
	{
		TestPoint p;
	...
	TEST_F(ExplicitRungeKutta4Test, testOdeQrCorrect)
	{
		TestPoint p;
	...
	TEST_F(ExplicitRungeKutta4Test, testIntegrate)
	{
		...
		while (test_data_ >> p)
		{
			ODE::StateVector const xplus = integrator_(ode_, p.t, p.x0, p.u, timeStep_);
