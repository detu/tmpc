...
namespace tmpc
{
	...
	public:
		 ...
		 */
		template <typename CondensedStage>
		void evalTo(CondensedStage& condensed) const
		{
			QpSize const cs = condensedQpSize(tmpc::qpSizeIterator(first_), tmpc::qpSizeIterator(last_));
			...
			Qc = first_->get_Q();
			submatrix(Rc, 0, 0, nu, nu) = first_->get_R();
			//K::left_cols(Sc, nu) = first_->get_S();
			submatrix(Sc, 0, 0, Sc.rows(), nu) = first_->get_S();
			qc = first_->get_q();
			//K::head(rc, nu) = first_->get_r();
			subvector(rc, 0, nu) = first_->get_r();

			//K::top_rows(Cc, nc) = first_->get_C();
			submatrix(Cc, 0, 0, nc, Cc.columns()) = first_->get_C();
			//K::top_left_corner(Dc, nc, nu) = first_->get_D();
			submatrix(Dc, 0, 0, nc, nu) = first_->get_D();
			//K::head(lbd, nc) = first_->get_lbd();
			subvector(lbd, 0, nc) = first_->get_lbd();
			//K::head(ubd, nc) = first_->get_ubd();
			subvector(ubd, 0, nc) = first_->get_ubd();

			//K::head(lbu, nu) = first_->get_lbu();
			subvector(lbu, 0, nu) = first_->get_lbu();
			//K::head(ubu, nu) = first_->get_ubu();
			subvector(ubu, 0, nu) = first_->get_ubu();

			blaze::DynamicMatrix<Scalar> A = first_->get_A();
			blaze::DynamicMatrix<Scalar> B = first_->get_B();
			blaze::DynamicVector<Scalar> b = first_->get_b();
			...
			condensed.set_lbx(first_->get_lbx());
			condensed.set_ubx(first_->get_ubx());
