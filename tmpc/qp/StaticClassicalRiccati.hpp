...
namespace tmpc
{
    ...
    ///
    template <typename R, size_t NX_, size_t NU_>
    class StaticClassicalRiccati
    {
    public:
        using Real = R;
        ...
        StaticClassicalRiccati(OcpTree const& graph)
        :   size_ {graph}
        ,   P_(num_vertices(graph))
        ,   p_(num_vertices(graph))
        ,   Lambda_(num_vertices(graph))
        ,   L_(num_vertices(graph))
        ,   l_(num_vertices(graph))
        ,   PA_(num_edges(graph))
        ,   PB_(num_edges(graph))
        ,   APA_(num_edges(graph))
        ,   Pb_p_(num_edges(graph))
        {
        }
    ...
    private:
        ...
        void vertexBackward(OcpVertex u, Qp const& qp) const
        {
            ...
            if (out_degree(u, graph()) > 0)
            {
                ...
                for (auto e : out_edges(u, graph()))
                {
                    auto const v = target(e, graph());
        ...
        mutable std::vector<blaze::SymmetricMatrix<blaze::StaticMatrix<Real, NX, NX, blaze::columnMajor>>> P_;
        mutable std::vector<blaze::StaticVector<Real, NX>> p_;
        mutable std::vector<blaze::StaticMatrix<Real, NU, NU, blaze::columnMajor>> Lambda_;
        mutable std::vector<blaze::StaticMatrix<Real, NU, NX, blaze::columnMajor>> L_;
        mutable std::vector<blaze::StaticVector<Real, NU>> l_;

        mutable std::vector<blaze::StaticMatrix<Real, NX, NX, blaze::columnMajor>> PA_;
        mutable std::vector<blaze::StaticMatrix<Real, NX, NU, blaze::columnMajor>> PB_;
        mutable std::vector<blaze::StaticMatrix<Real, NX, NX, blaze::columnMajor>> APA_;
        mutable std::vector<blaze::StaticVector<Real, NX>> Pb_p_;
    };
}