...
namespace tmpc :: hpipm
{
    ...
    :	ocp_qp_dim<Real>
    {
        ...
        template <tmpc::OcpSize Size>
		explicit OcpQpDim(Size const& size)
        :   OcpQpDim {static_cast<int>(num_vertices(size.graph()) - 1)}
		{
			if (size.graph().scenarioCount() > 1)
				TMPC_THROW_EXCEPTION(std::invalid_argument(
                    "OcpQpDim constructor needs a single-scenario tree"));
