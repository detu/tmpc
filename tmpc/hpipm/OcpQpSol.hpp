...
namespace tmpc :: hpipm
{
    ...
    :   ocp_qp_sol<Real>
    {
        ...
        void get_all(
            ...
            Real * const * pi, Real * const * lam_lb, Real * const * lam_ub, Real * const * lam_lg, Real * const * lam_ug,
            Real * const * lam_ls, Real * const * lam_us)
        {
        ...
        void get(char const * field, int stage, Real * vec)
        {
            ocp_qp_sol_get(field, stage, *this, vec);
        }

        
        void get_u(int stage, Real *vec) const
        {
            ocp_qp_sol_get_u(stage, *this, vec);
        }

        
        void get_x(int stage, Real *vec) const
        {
            ocp_qp_sol_get_x(stage, *this, vec);
        }

        
        void get_sl(int stage, Real *vec) const
        {
            ocp_qp_sol_get_sl(stage, *this, vec);
        }

        
        void get_su(int stage, Real *vec) const
        {
            ocp_qp_sol_get_su(stage, *this, vec);
        }

        
        void get_pi(int stage, Real *vec) const
        {
            ocp_qp_sol_get_pi(stage, *this, vec);
        }

        
        void get_lam_lb(int stage, Real *vec) const
        {
            ocp_qp_sol_get_lam_lb(stage, *this, vec);
        }

        
        void get_lam_ub(int stage, Real *vec) const
        {
            ocp_qp_sol_get_lam_ub(stage, *this, vec);
        }

        
        void get_lam_lg(int stage, Real *vec) const
        {
            ocp_qp_sol_get_lam_lg(stage, *this, vec);
        }

        
        void get_lam_ug(int stage, Real *vec) const
        {
            ocp_qp_sol_get_lam_ug(stage, *this, vec);
        }

        
        void set(char const * field, int stage, Real const * vec)
        {
            ocp_qp_sol_set(const_cast<char *>(field), stage, const_cast<Real *>(vec), *this);
        }

        
        void set_u(int stage, Real const * vec)
        {
            ocp_qp_sol_set_u(stage, const_cast<Real *>(vec), *this);
        }

        
        void set_x(int stage, Real const * vec)
        {
            ocp_qp_sol_set_x(stage, const_cast<Real *>(vec), *this);
        }

        
        void set_sl(int stage, Real const * vec)
        {
            ocp_qp_sol_set_sl(stage, const_cast<Real *>(vec), *this);
        }

        
        void set_su(int stage, Real const * vec)
        {
            ocp_qp_sol_set_su(stage, const_cast<Real *>(vec), *this);
        }
