function qp = qp_extract_axis(qp, j)
    ...
    for i = 1 : Nt
        ...
        qp.D{i} = qp.D{i}(x_ind, z_ind);
        qp.dMin{i} = qp.dMin{i}(x_ind);
        qp.dMax{i} = qp.dMax{i}(x_ind);
    end
    ...
    qp.D{end} = qp.D{end}(x_ind, x_ind);
    qp.dMin{end} = qp.dMin{end}(x_ind);
    qp.dMax{end} = qp.dMax{end}(x_ind);
end
