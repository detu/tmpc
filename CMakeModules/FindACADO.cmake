...
ELSE()
	...
	FOREACH( LIB $ENV{ACADO_ENV_STATIC_LIBRARIES} )
		IF(MSVC)
			UNSET( ACADO_TOOLKIT_STATIC_${LIB}_DEBUG )
			UNSET( ACADO_TOOLKIT_STATIC_${LIB}_RELEASE )
			
			FIND_LIBRARY( ACADO_TOOLKIT_STATIC_${LIB}_DEBUG
				NAMES ${LIB}
				PATHS ${ACADO_LIBRARY_DIRS}/Debug
				NO_DEFAULT_PATH
			)
		
			IF( ACADO_TOOLKIT_STATIC_${LIB}_DEBUG )
				MESSAGE( STATUS "Found ACADO static debug library: ${LIB}" )
				SET( ACADO_STATIC_LIBRARIES
					${ACADO_STATIC_LIBRARIES} debug ${ACADO_TOOLKIT_STATIC_${LIB}_DEBUG}
				)
			ELSE( )
				MESSAGE( STATUS "Could not find ACADO static debug library: ${LIB}" )
				SET( ACADO_STATIC_LIBS_FOUND FALSE )
			ENDIF( )
			
			FIND_LIBRARY( ACADO_TOOLKIT_STATIC_${LIB}_RELEASE
				NAMES ${LIB}
				PATHS ${ACADO_LIBRARY_DIRS}/Release
				NO_DEFAULT_PATH
			)
		
			IF( ACADO_TOOLKIT_STATIC_${LIB}_RELEASE )
				MESSAGE( STATUS "Found ACADO static release library: ${LIB}" )
				SET( ACADO_STATIC_LIBRARIES
					${ACADO_STATIC_LIBRARIES} optimized ${ACADO_TOOLKIT_STATIC_${LIB}_RELEASE}
				)
			ELSE( )
				MESSAGE( STATUS "Could not find ACADO static release library: ${LIB}" )
				SET( ACADO_STATIC_LIBS_FOUND FALSE )
			ENDIF( )
		ELSE()
